...
エディタで Enter を追加入力して生成した空白段落は、保存時に `.mdi` ファイルへ
`[[blank]]` として書き込まれる。
...
- `.mdi` モードのみ有効（`.md` ファイルでは通常テキストとして扱われる）
- round-trip で確実に保持される（末尾の空段落を除く）
- エクスポート時: TXT → 空行、HTML → `<p></p>`、DOCX → 空段落
- ユーザーが `[[blank]]` をリテラルテキストとして入力した場合、空白段落として解釈される（bracket macro 全般の既知 escape 制限と同様）
