...
| プラットフォーム | ショートカット | 動作                                                            |
| :--------------- | :------------- | :-------------------------------------------------------------- |
| macOS            | `⌘C`           | 選択テキストをコピー（選択がなければ SIGINT として PTY に渡す） |
| macOS            | `⌘V`           | クリップボードから貼り付け                                      |
| Windows / Linux  | `Ctrl+C`       | 選択テキストがあればコピー、なければ SIGINT として PTY に渡す   |
| Windows / Linux  | `Ctrl+Shift+C` | 選択の有無に関係なく常にコピー                                  |
| Windows / Linux  | `Ctrl+V`       | クリップボードから貼り付け                                      |
| Windows / Linux  | `Ctrl+Shift+V` | クリップボードから貼り付け（常に）                              |

macOS では、ブラウザのネイティブ `Cmd+C/V` パイプラインがそのまま使用されます。Windows/Linux では、`attachCustomKeyEventHandler` によって xterm.js のデフォルトキーイベント処理を上書きしています。

### 右クリックコンテキストメニュー

ターミナルパネル上で右クリックすると、Radix UI `ContextMenu` を使用したメニューが表示されます。

| アイテム     | 動作                                                       |
| :----------- | :--------------------------------------------------------- |
| コピー       | 現在の選択をクリップボードにコピー（選択がない場合は無効） |
| 貼り付け     | クリップボードの内容を PTY に貼り付け                      |
| すべて選択   | ターミナルバッファ全体を選択                               |
| 画面をクリア | xterm バッファをクリア                                     |

コピー・貼り付けのメニューには、使用中の OS に対応したショートカットキーが表示されます（macOS: `⌘C/⌘V`、その他: `Ctrl+C/Ctrl+V`）。
