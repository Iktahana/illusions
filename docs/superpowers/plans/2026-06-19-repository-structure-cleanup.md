...
- Ruleset framework PR #1795 is merged.
- Structural baseline: `dev` merge commit `c18396668c7cc7f76d230b862a0d4723ddd153e0`.
- Included commits: `98c0100`, `8cafaa2`, `d23e067`, `dc8cff7`, `7ea1afd`, and CI fix `1ea6249`.
- Canonical ruleset documentation: `docs/ruleset/`.
- Follow-up legacy rule migration is tracked by #1791/#1792.

Treat the PR #1795 framework as one versioned surface during structural moves:

1. `lib/linting/{sdk,registry,toolkit}/`, `external-ruleset-loader.ts`, and presets.
2. `lib/editor-page/use-linting.ts` plus worker proxy/protocol/worker.
3. `components/settings/LintingSettings.tsx` and `components/settings/linting/`.
4. Electron ruleset manager/IPC/preload/channels and `types/electron.d.ts`.
5. `docs/ruleset/` and external template contract.
...
1. Read and treat these as constraints, not suggestions:
   ...
   - `docs/ruleset/README.md`, `docs/ruleset/authoring.md`, and repository `AGENTS.md` for the merged ruleset SDK, manifest, toolkit, and dictionary fail-safe contracts.
...
3. Retain `joyo-kanji.ts` and `jinmeiyo-kanji.ts` until #1791/#1792 complete legacy rule migration. Re-audit them afterward; they are not Phase 1 deletion candidates.
...
3. Establish `packages/illusions-lint-sdk/` as the stable external `RulesetModule`/`RulesetContext` type contract. Copy the merged SDK contract rather than redesigning it, then update `docs/ruleset/` and the external template in the same release.
4. Create `features/proofreading/{model,ui,worker}/` from the application-owned lint registry/toolkit/rules, correction UI, lint hooks, external loader, and current Milkdown lint worker/plugin.
5. Move the PR #1795 runtime chain atomically: SDK consumer, registry/toolkit, external loader, `use-linting`, worker protocol/proxy/worker, settings UI, and Electron bridge contracts must remain version-aligned at every commit.
6. Move `pos-highlight/` to proofreading/editor integration because it imports application NLP/cache code.
7. Leave `packages/milkdown-plugin-japanese-novel/` with MDI/Milkdown syntax, schemas, formatting, and serialization only.
8. Create `features/dictionary/` from `lib/dict`, user-dictionary service, dictionary UI, and dictionary-owned startup integration.
9. Create `features/search/` from SearchDialog/SearchResults, match/search workers, and search-only state.
10. Preserve `docs/ruleset/` as the canonical authoring documentation. Update every path/API example atomically with the SDK relocation.
...
- `docs/ruleset/README.md`, `docs/ruleset/authoring.md`, and `docs/ruleset/structure.md`
...
- Run the PR #1795 registry, SDK, toolkit, external-loader, ruleset-manager, IPC drift, and ruleset settings UI tests after every framework move.
- Verify external ruleset load/unload correlation, failure-isolated runner swap, manifest integrity checks, and Web no-op behavior.
...
- Do not move only one side of the ruleset loader/worker/IPC/UI chain.
- Do not rename stable `ruleId`, `guidelineId`, correction mode, manifest, or worker protocol fields during a structure-only PR.
...
9. Standalone type-check for `packages/illusions-lint-sdk/` and its external template consumer.
10. Ruleset framework verification: registry/SDK/toolkit/loader/worker/UI/Electron tests plus dictionary-not-ready behavior.
11. Boundary checker with zero temporary allowlist entries.
12. Grep checks for old roots/paths in code, docs, `.github`, scripts, and configs.
13. Confirm no generated/local artifacts are tracked and `public/` contains only served assets.
...
The first implementation branch must fork from `c183966` or a later descendant of `dev` that contains PR #1795. Do not transplant the structure work onto the pre-merge base.
