...
```text
/make-plan Prepare the repository hygiene PR described by PATHFINDER-2026-06-19/02-duplication-report.md, based on dev merge commit c183966 (PR #1795). Delete only high-confidence legacy files, untrack docs/.obsidian workspace state, move editable PSD/AI sources from public/ to assets/source/branding/, and repair stale documentation references. Treat FileConflictDialog as verification-gated. Retain joyo-kanji.ts and jinmeiyo-kanji.ts until #1791/#1792 resolve legacy rule migration. Do not change runtime behavior, alter the merged ruleset framework, introduce compatibility shims, or delete public/dict and tracked build icons. Include type-check, lint, full tests, Next build, Electron bundle, and git-ls-files verification.
```
...
```text
/make-plan Split editor and proofreading ownership using PATHFINDER-2026-06-19/01-flowcharts/editor-mdi.md and proofreading-nlp.md, with merged PR #1795 at c183966 as the baseline. Treat lib/linting/{sdk,registry,toolkit}, external-ruleset-loader, use-linting wiring, worker proxy/protocol/worker, ruleset settings UI, Electron ruleset IPC/manager/preload/channels/types, and docs/ruleset as one versioned framework surface. Establish packages/illusions-lint-sdk for the stable external module/context types, updating the external template/docs atomically. Move the application-coupled worker/plugin and pos-highlight out of the Milkdown package into features/proofreading/. Preserve failure-isolated runner swap, load/unload correlation, integrity checks, dictionary-not-ready warning/disable behavior, ctx.toolkit usage, and code-module distribution. Do not duplicate engines, hardcode dictionary mappings, or move only one side of the runtime chain.
```
