...
export default function CreateProjectWizard({
  ...
  const [selectedExtension, setSelectedExtension] = useState<SupportedFileExtension>(".mdi");
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      const project = await projectService.createProject(projectName, selectedExtension);
      ...
      <div className="mb-6 flex items-center justify-center gap-3">
        {STEP_LABELS.map((label, index) => (
          <div key={label} className="flex items-center gap-3">
            ...
            {index < STEP_LABELS.length - 1 && <div className="h-px w-8 bg-border" />}
          </div>
        ))}
      </div>
      ...
      {step === "name-format" && (
        <div>
          <div className="mb-1 flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-foreground">新規プロジェクト</h2>
          </div>
          ...
          <div className="mb-5">
            ...
            {projectName && !isNameValid && nameValidation.error && (
              <p className="mt-1 text-xs text-red-500">{nameValidation.error}</p>
            )}
          </div>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-foreground">MDI形式（推奨）</span>
              ...
              {showOtherFormats &&
                FORMAT_OPTIONS.filter((opt) => !opt.recommended).map((option) => (
                  <label
        ...
        <div className="flex flex-col items-center py-4">
          ...
          {isCreating && (
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="text-sm text-foreground-secondary">プロジェクトを作成しています...</p>
            </div>
          )}
          ...
          {errorMessage && !isCreating && !creationSuccess && (
            <div className="flex w-full flex-col items-center gap-4">
              ...
              <p className="text-center text-sm text-foreground-secondary">{errorMessage}</p>
