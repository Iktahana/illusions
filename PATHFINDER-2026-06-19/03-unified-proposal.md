...
7. `packages/milkdown-plugin-japanese-novel/` has no `@/` application imports and passes its own `tsconfig.json` independently.
8. Each feature exposes a small root `index.ts`; internal imports use relative paths. Avoid repository-wide barrels.
...
| `lib/linting`, lint worker/plugin, corrections UI                                   | `features/proofreading/`                                                                                                                            |
