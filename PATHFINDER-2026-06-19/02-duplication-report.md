...
| Old kanji data modules                                  | `lib/linting/data/joyo-kanji.ts` and `jinmeiyo-kanji.ts` have no code consumers, but `docs/guides/linting-rules.md:249-250` advertises them.                                                                                                                                                                                                                             | Verification-gated deletion or explicit reactivation; do not leave undocumented dead data. | Medium because current docs claim intentional availability. |
...
### 3. Proofreading worker bootstrap repeated inside an editor package

- Main-thread proxy imports and constructs the application rule engine: `packages/milkdown-plugin-japanese-novel/linting-plugin/worker/rule-runner-proxy.ts:10-18,79-86`.
- Worker entry separately imports and constructs the same application engine: `packages/milkdown-plugin-japanese-novel/linting-plugin/worker/linting.worker.ts:13-17`.

The worker/main split is legitimate; placing both inside the standalone Milkdown package while importing `@/lib/*` is not. Move the complete lint editor adapter and worker into `features/proofreading/`, leaving the Milkdown package application-independent.
