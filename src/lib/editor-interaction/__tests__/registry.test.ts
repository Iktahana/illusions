...
describe("editor command registry", () => {
  ...
  it("builds context menu enabled state from availability", () => {
    ...
    expect(menu).toHaveLength(EDITOR_CONTEXT_MENU.length);
    ...
    expect(menu).toContainEqual({ command: "speech.toggle", enabled: false });
    expect(menu).toContainEqual({ command: "speech.stop", enabled: false });
